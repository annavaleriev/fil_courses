...
class LessonTestCase(APITestCase):  # Тесты для уроков
    def setUp(self):  # Создаем клиент
        ...
        )  # Получаем url для списка уроков
    ...
    def create_lesson(
        ...
        response = self.client.post(
            ...
            data={  # Отправляем POST запрос на создание урока
                "title": title,
                "video": video,
                "course": course.pk,
            },
...
class ListLessonTestCase(LessonTestCase):  # Тесты для списка уроков
    def test__get_lessons__super_user(self):  # Получаем уроки для суперпользователя
        ...
        response = self.client.get(
            self.lesson_list_url
        )  # Отправляем GET запрос на список уроков
        ...
        expected_lesson_ids = {
            lesson["id"] for lesson in response.data["results"]
        }  # Получаем id уроков из ответа
    ...
    def test__get_lessons__owner_user(self):  # Получаем уроки для владельца
        ...
        user_owner_lessons = (
            LessonFactory.create_batch(  # Создаем уроки для пользователя
        ...
        self.assertEqual(
            response.data["count"], count_user_owner_lessons
    ...
    def test__get_lessons__moder_user(self):  # Получаем уроки для модератора
        ...
        not_owner_lessons = LessonFactory.create_batch(
            count_not_owner_lessons
        )  # Создаем уроки не принадлежащие пользователю
        user_owner_lessons = (
            LessonFactory.create_batch(  # Создаем уроки для пользователя
                count_user_owner_lessons, owner=user  # Создаем уроки для пользователя
            )
        ...
        self.assertEqual(  # Проверяем количество уроков
            response.data["count"],
            count_user_owner_lessons
            + count_not_owner_lessons,  # Проверяем количество уроков
        )
...
class DeleteLessonTestCase(LessonTestCase):
    ...
    def test__delete_lesson__super_user(self):
        ...
        response = self.client.delete(
            reverse("materials:lesson-detail", args=[lesson.pk])
        )  # Отправляем DELETE запрос на удаление урока
        self.assertEqual(
